...
public class MyCardGame
{
    public static void main( String[] args)
    {
        ...
        for (int i = 0; i < game.getPlayerList().size(); i++) {
            if (game.getScoreCard()[i] > winner) {
                winner = game.getScoreCard()[i];
                p = game.getPlayerList().get(i);
            }
        }

        System.out.println("Tha winner is: " + p.getName() + "!");        
        System.out.println( "\nEnd of MyCardGame\n" );   
    }
    ...
    private static boolean play( Player p, CardGame game)
    {
        ...
        accepted = game.isTurnOf(p);
        if (accepted) {
            c = p.getCards().getTopCard();
            game.getCardsOnTable()[game.getRoundNo()].addTopCard(c);
            game.incrementTurnOfPlayerNo(1);

            // System.out.println(game.getCardsOnTable()[game.getRoundNo()].toString());

            if (game.getTurnOfPlayerNo() == 4) {
                game.getWinners();
                game.incrementTurnOfPlayerNo(-4);
                game.incrementRoundNo(1);
            }
        }
